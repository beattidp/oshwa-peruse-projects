...
To sign up for an API token, just a first and last name and email address are required. This is achieved with ``oshwa_api_signup.py``. The retrieved token is written to a file at ``~/.oshwa_token``.

The full set of project data is then retrieved using ``oshwa_get_all_projects.py`` and written to a local file ``./cache/oshwa_projects.json``.

At that point, assuming Playwright and chrome-headless or other browser of choice _(installed by ``playwright install``)_ are present, the application ``browse_oshwa.py`` can be run.
